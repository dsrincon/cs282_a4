...
class QLearner(object):
    ...
    def __init__(self,
        ...
        # Define action predicted by Q placeholder
        self.act_t_ph_nxt     = tf.placeholder(tf.int32,   [None])
        ...
        # ----------------------------------------------------------------------
        # ----------------------------------------------------------------------
        # DONE: START OF YOUR CODE
        # ----------------------------------------------------------------------

        q_t=q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False)
        q_x=q_func(obs_t_float, self.num_actions, scope="q_func", reuse=False)

        # Main Q network predictions

        self.action_selected=tf.argmax(q_x, axis=-1, name='action_selected')#Store for future action selection
        action_oh=tf.one_hot(self.act_t_ph,self.num_actions,1.0,0.0,name='action_oh')
        x=tf.reduce_sum(q_x*action_oh,axis=-1,name='q_pred')

        # Target values from optimal policy + Bellman

        if self.double_q: # Change observation based on action that optimizes Q and not target Q
            #act_fl = self.act_t_ph_nxt + tf.range(0, self.batch_size) * q_t.shape[1]
            #max_q_n_t = tf.gather(tf.reshape(q_t, [-1]), act_fl)

            act_fl=tf.one_hot(self.act_t_ph_nxt,self.num_actions,1.0,0.0,name='action_nxt')
            max_q_n_t=tf.reduce_sum(q_t*act_fl,axis=-1,name='q_t')
            y = self.rew_t_ph+(1.0-self.done_mask_ph)*gamma*max_q_n_t

        else:

            max_q_t=tf.reduce_max(q_t,axis=-1)
            y=self.rew_t_ph+(1.0-self.done_mask_ph)*gamma*max_q_t

        # Calculate loss
        self.total_error=tf.reduce_mean(huber_loss(tf.stop_gradient(y)-x),name="huber_loss")
    ...
    def step_env(self):
        ...
        # Retrieve encoded obs with context
        obs_ctx=self.replay_buffer.encode_recent_observation()


        # Check model init if not choose random if it is on

        if self.model_initialized:  # If initialized run strategy

            # Input obs_ctx to Q network to get optimal action
            opt_action=self.session.run(self.action_selected,{self.obs_t_ph:obs_ctx[None]})
            # Sample random action
            rand_action=self.env.action_space.sample()

            # Greedy exploration action selection
            epsilon=self.exploration.value(self.t) #epsilon for greedy exploration

            select=np.random.binomial(1,(1.0-epsilon))
            action=(1-select)*rand_action+select*opt_action
            action=action[0]



        else: # If not select random action
            action=self.env.action_space.sample()


        # Get data from environment after action
        obs, reward, done, info = self.env.step(action)
        ...
        # Store effect of observations
        self.replay_buffer.store_effect(idx, action, reward, done)

        # Update observation in QLearner
        self.last_obs=obs
    ...
    def update_model(self):
        ...
        if (self.t > self.learning_starts and \
            ...
            # 3.a Use replay buffer to sample a batch transitions

            (obs_batch,act_batch,rew_batch,next_obs_batch,done_mask)=self.replay_buffer.sample(self.batch_size)

            nxt_action=act_batch


            # 3.b Initialize model and copy to target Q network
            ...
            if not self.model_initialized:
                ...
                # Copy initialized variables from Q to Q-target_q_func
                self.update_target_fn

                # Change model initialization flag
            ...
            if self.double_q: # Get action for double DQ
                nxt_action=self.session.run(self.action_selected,{self.obs_t_ph:next_obs_batch})


            feed_val={self.obs_t_ph:obs_batch,self.act_t_ph:act_batch,self.rew_t_ph:rew_batch,\
            self.obs_tp1_ph:next_obs_batch,self.done_mask_ph:done_mask,self.learning_rate:learning_rate,\
            self.act_t_ph_nxt:nxt_action}

            _=self.session.run(self.train_fn,feed_dict=feed_val)

            # 3.d Update target network when replays is equal to update frequency
