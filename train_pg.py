...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            # DONE: START OF YOUR CODE
            # ------------------------------------------------------------------
            sy_logits_na=build_mlp(sy_ob_no, self.ac_dim, "policy_forward_pass_dis", self.n_layers, self.size,
                            activation=tf.tanh, output_activation=None)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
            ...
            # DONE: START OF YOUR CODE
            # ------------------------------------------------------------------
            sy_mean=build_mlp(sy_ob_no, self.ac_dim, "policy_forward_pass_mean", self.n_layers, self.size,
                            activation=tf.tanh, output_activation=None)

            sy_logstd=tf.get_variable(shape=[self.ac_dim], name="sy_logstd", dtype=tf.float32)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # DONE: START OF YOUR CODE
            # ------------------------------------------------------------------
            disc_dist=tf.distributions.Categorical(logits=sy_logits_na, name="disc_dist")
            sy_sampled_ac=disc_dist.sample()
            ...
            # DONE: START OF YOUR CODE
            # ------------------------------------------------------------------
            normz_dist=tf.distributions.Normal(loc=0.0,scale=1.0)
            normz_sample=normz_dist.sample(sample_shape=tf.shape(sy_mean))
            sy_sampled_ac=sy_mean+tf.multiply(sy_logstd,normz_sample)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            # DONE: START OF YOUR CODE
            # ------------------------------------------------------------------
            sy_logprob_n=tf.distributions.Categorical(logits=sy_logits_na, name="disc_dist").log_prob(sy_ac_na)
            ...
            z=tf.multiply((sy_ac_na-sy_mean),1/sy_logstd)
            #sy_logprob_n=tf.distributions.Normal(loc=0.0,scale=1.0).log_prob(z)
            sy_logprob_n = tf.contrib.distributions.MultivariateNormalDiag(
                loc=sy_mean, scale_diag=tf.exp(sy_logstd)).log_prob(sy_ac_na)
            #func=lambda x:normz_dist.log_prob(x)
            #sy_logprob_n=tf.map(func,z)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
        return sy_logprob_n
    ...
    def build_computation_graph(self):
        ...
        # DONE: START OF YOUR CODE
        # ------------------------------------------------------------------
        sy_adv_n_f=tf.cast(self.sy_adv_n, tf.float32)
        sy_logprob_n_f=tf.cast(self.sy_logprob_n, tf.float32)
        self.loss=tf.tensordot(-sy_logprob_n_f,sy_adv_n_f,axes=1)
        ...
        if self.reward_to_go:
            for i in range(len(re_n)):
                for j in range(len(re_n[i])):
                    ...
                    gammas=np.ones(length)*self.gamma
                    powers=np.arange(length)
                    discount=np.power(gammas,powers)
                    q_ij=np.sum(re_n[i][j:]*discount,axis=0)
                    q_n.append(q_ij)
            ...
            for i in range(len(re_n)):
                for j in range(len(re_n[i])):
                    #q_ij=tf.reduce_sum(re_n[i]*(tf.pow(self.gamma,j)),axis=0)
                    gammas=np.ones(len(re_n[i]))*self.gamma
                    powers=np.arange(len(re_n[i]))
                    discount=np.power(gammas,powers)
                    q_ij=np.sum(re_n[i]*discount,axis=0)
                    q_n.append(q_ij)
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        feed_val={self.sy_ob_no:ob_no, self.sy_ac_na:ac_na, self.sy_adv_n:adv_n}
        #self.loss_pre=self.sess.run(self.loss,feed_dict=feed_val) #loss before update
        self.sess.run(self.update_op,feed_dict=feed_val)
        #self.loss_pos=self.sess.run(self.loss,feed_dict=feed_val) #loss after update
